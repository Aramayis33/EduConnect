...
public interface ScheduleRepository extends JpaRepository<Schedule, Integer> {
    ...
    @Query("Select s From Schedule s Where s.classroom=:classroom And s.semester=:semester and s.weekday=:weekday and s.classHour=:classHour " +
            "And (s.semesterPart = :semesterPart OR s.semesterPart = 'all') ")
    Schedule findLaboratoryUsage(@Param("semester")int semester,@Param("semesterPart")String semesterPart,
                                 @Param("weekday")int weekday,@Param("classHour")int classHour,
                                 @Param("classroom")String classroom);


    @Query("SELECT s FROM Schedule s WHERE s.teacher = :teacher AND s.semester = :semester " +
            "AND (s.semesterPart = :semesterPart OR s.semesterPart = 'all')")
    List<Schedule> findSchedulesByTeacherAndSemesterAndSemesterPartOrAll(
    ...
    @Query("SELECT s FROM Schedule s Where s.teacher= :teacher AND s.group= :group And s.semester= :semester And (s.semesterPart = :semesterPart OR s.semesterPart = 'all') And s.subject= :subject ")
